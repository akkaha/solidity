...
contract BlindAuction {
    ...
    /// `onlyBefore` is applied to `bid` bellow:
    ...
    /// The bid is valid if the ether send togother with the bid is at least "value" and "fake" is not true.
    /// Setting "fake" to true and sending not the exact amount are ways to hide the real bid but still
    /// make the required deposit. The same address can place multiple bids.
    ...
    /// Reveal your blinded bids. You will get a refund for all correctly blinded bids and for all bids
    ...
    function withdraw() public {
        ...
        if (amount > 0) {
            ...
            // again as part of the receiving call before `transfer` returns
            // (see the remark above about conditions -> effects -> interactino).
    ...
    function auctionEnd() public onlyAfter(revealEnd) {
        ...
        ended = true;
        beneficiary.trasfer(highestBid);
    }
}
