...
contract SimpleAuction {
    ...
    // The following is a so-calle natspec commnet, recognizable by the three slashes.
    ...
    /// Bid on the auction with the value sent togother with this transaction.
    ...
    function bid() public payable {
        ...
        require(now <= auctionEnd, "Aution already ended.");

        // If the bid is not higher, send the money back.
        require(msg.value > highestBid, "There already is a higher bid.");

        if (highestBid != 0) {
            // Sending back the money by simply using highestBidder.send(highstBid) is a 
            // security rist because it could execute an untrusted contract.
    ...
    // Withdraw a bid what was overbid.
    ...
    function auctionEnd() public {
        // It is a good guideline to structure functoins that interact with other contracts (i.e. they call 
        // functions or send Ether) into three phases:
        // 1. checking conditions
        // 2. performing actions (potentially changing conditions)
        // 3. interacting with other contracts
        // If these phases are mixed up, the other contract could call back into the current contract and modify
        // the state or cause effects (ether payout) to be performed multiple times.
        // If functions called internally include interaction with external contracts, they also have to be considered
